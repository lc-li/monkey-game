...
export default class List extends cc.Component {
  ...
  private _j_listItemRendedTime: number = 0;
  //列表数量
  @property({
    serializable: false
  })
  ...
  set numItems(val: number) {
    ...
    t._j_listItemRendedTime = new Date().getTime();
  }
  get numItems() {
  ...
  //----------------------------------------------------------------------------
  /**liangzhi:滚动结束触发事件，找到最近的点，并且回调数据 */
  callbackScrollEnded: any
  onLoad() {
    this._init();
  }
  ...
  _onScrollEnded() {
    ...
    t._onScrolling();
    if (this.callbackScrollEnded) {//有回调，找到最近的位置并且返回
      this.touchScrollEnded()
      return
    }
  ...
  /**liangzhi:暂时只有单列，其他的没有用到，hhh */
  touchScrollEnded() {
    let t = this;
    let data = t._virtual ? t.displayData[0] : t._calcExistItemPos(0);
    if (data) {
      let x = t.viewLeft + t.node.width / 2;//x坐标中心点位置  (data.left + data.right) / 2
      let nearItem = [-1, 0];
      for (let i = 0; i < t.displayData.length; i++) {
        const item = t.displayData[i];
        let value = Math.abs(item.x - x)
        if (nearItem[0] == -1 || value < nearItem[1]) {//找到最近位置
          nearItem[0] = i;
          nearItem[1] = value;
        }
      }
      let id = t.displayData[nearItem[0]].id;//id为排列顺序号，从0开始
      this.callbackScrollEnded(id)
    }
  }
  // 触摸时
  ...
  _createOrUpdateItem(data: any) {
    ...
    if (!item) { //如果不存在
      ...
      if (this.renderEvent) {
        ...
        if (this._j_listItemRendedTime > 0 && new Date().getTime() - this._j_listItemRendedTime < 300) {
          // cc.Tween.stopAllByTarget(item);
          // if(item.getComponent(ButtonEx)) item.getComponent(ButtonEx).enabled = false;
          // item.opacity = 0;
          // const y = item.y;
          // item.y -= 100;
          // cc.tween(item).delay(0.02*data.id % this._actualNumItems).to(0.02, {opacity: 255, y: y}).call(()=>{ if(item.getComponent(ButtonEx)) item.getComponent(ButtonEx).enabled = true; }).start();
          this._j_listItemRendedTime = new Date().getTime();
        } else {
          this._j_listItemRendedTime = 0;
        }
      }
   ...
   * @returns
   */
  getListIdByItem(item) {
    if (this.content) {
      return this.content.children.indexOf(item);
    }
    return -1;
  }
  /**
