...
export default class ListItem extends cc.Component {
    ...
    private _btnCom: any;
    @property()
    is_touchEvent: boolean = false;//是否是点击事件
    get btnCom() {
        if (!this._btnCom)
    ...
    touchTime: number = 15;//触摸时间
    touchScope: number = 50;//触摸范围
    start_touch: any = null;
    touchSeedTime: { time: number, is_touch: boolean } = { time: 0, is_touch: false }
    isItemTouch = 0;//是否是物品的触摸
    onLoad() {
    ...
    _registerEvent() {
        if (!this._eventReg) {
            if (!this.is_touchEvent) {
                ...
                this.touchScope = (this.node.width + this.node.height) / 2;//触摸范围
                this.addTouch()
            }
    ...
    onTouchStart(event) {
        this.isItemTouch = 1;
        this.touchSeedTime.is_touch = true;
        this.touchSeedTime.time = 0;
        this.start_touch = event.getLocation()
    }

    onTouchEnd(event) {
        this.isItemTouch = 2;
        if (this.touchSeedTime.is_touch) {
            this.touchSeedTime.is_touch = false;
            this.onClickThis()
        }
    }
    protected update(dt: number): void {
        if (this.touchSeedTime.is_touch) {
            this.touchSeedTime.time += 1;
        }
    }
}
